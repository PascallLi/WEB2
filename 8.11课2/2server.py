...
class Message(object):
    ...
    def __repr__(self):  # repr() 的作用是产生机器可读的输出（大部分情况下，其输出可以作为有效的Python代码）
        return '{}: {}'.format(self.author, self.message)


message_list = []  # 要做一个留言板
request = Request()
...
def template(name):
    with open(name, 'r', encoding='utf-8') as f:  # 默认在中文windows下面打开的编码格式是gbk,所以需要utf-8
        return f.read()
...
def route_message():
    ...
    msgs = '<br>'.join([str(m) for m in message_list]) # str(m)一下然后回车，遍历list然后生成一个新的字符串
    # br是HTML中的回车
