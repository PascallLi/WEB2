        ...
        <div class='edit-comment'>
          ...
          <button class="weibo-delete" data-id= ${ w.id}>删除</button>
            <div class='comment-api'>
                  <input type="hidden" class="weibo_id" name="weibo_id" value= ${w.id} >
      ...
      var bindEvents = function() {
       ...
       var bindEventCommentToggle = function() {
          $('a.post-title-link').on('click', function(){
              // 对编辑选项进行点击
              console.log('触发3')
              // $(this).closest('.comment-api').slideToggle()
              $(this).closest('.edit-comment').find('.comments').slideToggle()
              // 输入框点击隐藏，a属性会跳转，默认阻止
              return false;
        })
      }
        ...
        var bindEventWeiboComment = function() {
           $('body').on('click', '.power-comment-button', function(){
               console.log('评论触发成功')
               var button = $(this)
               $('.gua-active').removeClass('gua-active')
               button.toggleClass('gua-active')
               var content = button.closest('.edit-comment').find('.power-comment-content').val()
               // var content = $(this).val()
               // var content = $('.power-comment-content').val()
               // var id = $(this).closest('.comment-api').find('.weibo_id').val()
               var id = button.closest('.edit-comment').find('.weibo-delete').data('id')
               ...
               var response = function(r){
                ...
                if (r.success) {
                 var c = r.data
                 button.closest('.edit-comment').find('.comments').append(weiboComment(c))
                 alert('评论成功')
                }
                else {
                 alert(r.message)
